 ...
 *
 * Feature-specific API functions (getHomepage, getLanguages, etc.) are provided
...
/**
 * Helper function to create API request objects with auto-filled URL
 *
 * Eliminates redundancy by automatically filling the URL property from the type's literal.
 * This allows callers to omit the URL when creating request objects.
 *
 * @param url - The endpoint URL (literal from the *Data type)
 * @param data - Request data without the URL property
 * @returns Complete request object with URL
 *
 * @example
 * ```typescript
 * // Instead of: { body: { email, password }, url: '/auth/login' }
 * const request = createApiRequest('/auth/login', { body: { email, password } })
 * ```
 */
export function createApiRequest<T extends ApiRequest>(
  url: T extends { url: infer U } ? U : never,
  data: Omit<T, 'url'>
): T {
