...
class OpenAPIContractLoader:
    ...
    def __init__(self, contract_path: str | Path = "/contracts/affilibuster.openapi.yaml"):
        """
    ...
    def load_contract(self) -> dict[str, Any]:
        ...
        try:
            logger.info(f"Loading OpenAPI contract from {self.contract_path}")
            with open(self.contract_path) as f:
                spec = yaml.safe_load(f)

            if not isinstance(spec, dict):
                raise ValueError("OpenAPI contract must be a dictionary")
    ...
    def get_spec(self) -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.warning(f"Could not load OpenAPI contract: {e}")
            return None
