...
class TestGetCurrencies:
    ...
    async def test_get_currencies_returns_200(self, integration_client, strapi_test_data):
        """Test that GET /currencies returns 200 status code."""
        response = await integration_client.get("/v1/currencies")
        assert response.status_code == 200

    async def test_get_currencies_returns_data_structure(self, integration_client, strapi_test_data):
        ...
        returned_codes = {currency.get("code") for currency in data["data"]}
    ...
    async def test_get_currencies_with_pagination(self, integration_client, strapi_test_data):
    ...
    async def test_get_currencies_with_filters(self, integration_client, strapi_test_data):
    ...
    async def test_get_currencies_with_field_selection(self, integration_client, strapi_test_data):
    ...
    async def test_get_currencies_with_sorting(self, integration_client, strapi_test_data):
    ...
    async def test_get_currencies_verifies_usd_data(self, integration_client, strapi_test_data):
        ...
        assert usd.get("name") == expected_usd.name
        assert usd.get("symbol") == expected_usd.symbol
        assert usd.get("decimalPlaces") == expected_usd.decimal_places
        assert usd.get("isActive") == expected_usd.is_active
    ...
    async def test_get_currencies_strapi_error_returns_502(self, integration_client):
...
class TestGetCurrency:
    ...
    async def test_get_currency_by_id_returns_200(self, integration_client, strapi_test_data):
        ...
        if list_response.status_code == 200:
            ...
            if data.get("data") and len(data["data"]) > 0:
                currency_id = str(data["data"][0].get("id") or data["data"][0].get("documentId", "1"))
    ...
    async def test_get_currency_with_field_selection(self, integration_client, strapi_test_data):
    ...
    async def test_get_currency_with_invalid_id_returns_error(self, integration_client, strapi_test_data):
    ...
    async def test_get_currency_strapi_error_returns_502(self, integration_client, strapi_test_data):
        """Test that Strapi errors return 502 Bad Gateway."""
        response = await integration_client.get("/v1/currencies/1")
        # Should either succeed or fail with 502 if Strapi has issues
        assert response.status_code in [200, 404, 502]

    async def test_get_currency_path_parameter_validation(self, integration_client, strapi_test_data):
...
class TestCurrenciesErrorHandling:
    ...
    async def test_get_currencies_handles_strapi_connection_error(self, integration_client):
        """Test that connection errors to Strapi are handled gracefully."""
        response = await integration_client.get("/v1/currencies")
        # Should return either 200 (success) or 502 (Bad Gateway on error)
        assert response.status_code in [200, 502]

    async def test_get_currency_handles_strapi_connection_error(self, integration_client):
        """Test that connection errors to Strapi are handled gracefully."""
        response = await integration_client.get("/v1/currencies/1")
