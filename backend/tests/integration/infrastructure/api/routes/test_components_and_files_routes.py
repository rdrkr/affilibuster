...
class TestComponentsRoute:
    ...
    async def test_get_components_returns_200(self, integration_client: AsyncClient, strapi_test_data: None) -> None:
        """Test that GET /components returns 200 status code."""
        response = await integration_client.get("/v1/components")
        assert response.status_code in [200, 502]

    async def test_get_components_returns_data_structure(
    ...
    async def test_get_component_by_uid(self, integration_client: AsyncClient, strapi_test_data: None) -> None:
    ...
    async def test_get_component_by_uid_with_valid_uid(
        self, integration_client: AsyncClient, strapi_test_data: None
    ) -> None:
        ...
        if list_response.status_code == 200:
            data = ComponentsGetResponse(**list_response.json())
            if data.data and len(data.data) > 0:
                # Try to get first component's UID
                component_uid = data.data[0].uid or "test.component"
                response = await integration_client.get(f"/v1/components/{component_uid}")
                assert response.status_code in [200, 404, 502]

    async def test_get_component_with_invalid_uid_returns_error(
        self, integration_client: AsyncClient, strapi_test_data: None
    ) -> None:
    ...
    async def test_get_components_strapi_error_returns_502(self, integration_client: AsyncClient) -> None:
        """Test that Strapi errors return 502 Bad Gateway."""
        response = await integration_client.get("/v1/components")
        # Should either succeed (200) or fail with 502 if Strapi is down
        assert response.status_code in [200, 502]

    async def test_get_component_strapi_error_returns_502(self, integration_client: AsyncClient) -> None:
...
class TestFilesRoute:
    ...
    async def test_get_files_returns_200(self, integration_client: AsyncClient, strapi_test_data: None) -> None:
        """Test that GET /files returns 200 status code."""
        response = await integration_client.get("/v1/files")
        assert response.status_code in [200, 502]

    async def test_get_files_returns_data_structure(
        self, integration_client: AsyncClient, strapi_test_data: None
    ) -> None:
    ...
    async def test_get_files_with_pagination(self, integration_client: AsyncClient, strapi_test_data: None) -> None:
    ...
    async def test_get_files_with_filters(self, integration_client: AsyncClient, strapi_test_data: None) -> None:
    ...
    async def test_get_files_with_sorting(self, integration_client: AsyncClient, strapi_test_data: None) -> None:
    ...
    async def test_get_file_by_id(self, integration_client: AsyncClient, strapi_test_data: None) -> None:
        ...
        if list_response.status_code == 200:
            data = FilesGetResponse(list_response.json())
            files_list = data.root
            if files_list and len(files_list) > 0:
                file_id = str(files_list[0].id or "1")
                response = await integration_client.get(f"/v1/files/{file_id}")
                assert response.status_code in [200, 404, 502]

    async def test_get_file_with_field_selection(self, integration_client: AsyncClient, strapi_test_data: None) -> None:
    ...
    async def test_get_file_with_invalid_id_returns_error(
        self, integration_client: AsyncClient, strapi_test_data: None
    ) -> None:
    ...
    async def test_get_files_strapi_error_returns_502(self, integration_client: AsyncClient) -> None:
        """Test that Strapi errors return 502 Bad Gateway."""
        response = await integration_client.get("/v1/files")
        # Should either succeed (200) or fail with 502 if Strapi is down
        assert response.status_code in [200, 502]

    async def test_get_file_strapi_error_returns_502(self, integration_client: AsyncClient) -> None:
...
class TestComponentsAndFilesErrorHandling:
    ...
    async def test_components_handles_strapi_connection_error(self, integration_client: AsyncClient) -> None:
        """Test that connection errors to Strapi are handled gracefully."""
        response = await integration_client.get("/v1/components")
        # Should return either 200 (success) or 502 (Bad Gateway on error)
        assert response.status_code in [200, 502]

    async def test_component_by_uid_handles_strapi_connection_error(self, integration_client: AsyncClient) -> None:
        """Test that connection errors to Strapi are handled gracefully."""
        response = await integration_client.get("/v1/components/test.component")
        # Should return either 200/404 (success) or 502 (Bad Gateway on error)
        assert response.status_code in [200, 404, 502]

    async def test_files_handles_strapi_connection_error(self, integration_client: AsyncClient) -> None:
        """Test that connection errors to Strapi are handled gracefully."""
        response = await integration_client.get("/v1/files")
        # Should return either 200 (success) or 502 (Bad Gateway on error)
        assert response.status_code in [200, 502]

    async def test_file_by_id_handles_strapi_connection_error(self, integration_client: AsyncClient) -> None:
        """Test that connection errors to Strapi are handled gracefully."""
        response = await integration_client.get("/v1/files/1")
        # Should return either 200/404 (success) or 502 (Bad Gateway on error)
        assert response.status_code in [200, 404, 502]
