...
export default async function manifest(): Promise<MetadataRoute.Manifest> {
  ...
  return {
    name: navigation?.siteTitle ?? 'GentleHawk',
    short_name: navigation?.siteTitle ?? 'GentleHawk',
    description: navigation?.siteDescription ?? 'Your trusted source for curated products.',
    start_url: '/',
    display: 'standalone',
